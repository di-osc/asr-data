...
- Add `src/audio/stream.rs`: share continuous interleaved resampling between full loads and streams.
...
- [x] **Step 1: Replace normalization tests with failing sanitation and API-removal tests**
...
uv run --with pytest python -m pytest tests/test_bindings.py -k 'normalization_api_is_removed' -q
```
...
- [x] **Step 3: Remove normalization and implement sanitation**
...
- [x] **Step 1: Add failing option tests**
...
uv run --with pytest python -m pytest tests/test_bindings.py -k 'source_load_options' -q
```

Expected: compile/signature failures because the options do not exist.

- [x] **Step 3: Implement load options**
...
- [x] **Step 1: Add failing stream tests**
...
```bash
uv run --with pytest python -m pytest tests/test_bindings.py -k 'source_stream_options or source_stream_default_chunk' -q
```

Expected: signature failures.

- [x] **Step 3: Implement the transformed iterator**
...
- [x] **Step 1: Add failing async tests**
...
```bash
uv run --with pytest python -m pytest tests/test_bindings.py -k 'aload_options or astream' -q
```

Expected: missing parameter and missing method failures.

- [x] **Step 3: Implement async APIs**
...
- [x] **Step 1: Update type declarations**
...
- [x] **Step 2: Rebuild the Python extension**
...
git status --short
git diff --stat
git add src/audio/data.rs src/audio/mod.rs src/audio/decode.rs src/audio/stream.rs src/media.rs src/python.rs asr_data/__init__.py asr_data/_native.pyi tests/data_behaviour.rs tests/test_bindings.py README.md docs/superpowers/plans/2026-07-22-audio-source-load-stream-options.md
